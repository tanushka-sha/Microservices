...
import { imageAPI, tokenManager } from '../services/api'
...
export default function ImageGenPage({ onSubmit, userName, onLogout, onNavigateToDashboard }) {
  ...
  const handleGenerateImage = async (e) => {
    ...
    try {
      ...
      console.log('Calling image generation API with prompt:', text)
      ...
      <main className="relative flex-1 p-4 sm:p-6">
        ...
        <div className="h-full w-full flex items-center justify-center">
          <div className="w-full max-w-3xl space-y-4">
            <h1 className="text-3xl md:text-5xl font-bold">AI Image Generator</h1>
            <p className={`${isDark ? 'text-slate-300' : 'text-slate-700'} text-lg`}>Generate an image by describing what you want to see, all images are published publicly by default.</p>

            <form onSubmit={handleGenerateImage} className="flex flex-col sm:flex-row items-center gap-4 pt-2">
              ...
              <button
                type="submit"
                className="rounded-xl bg-sky-500 px-5 py-3 text-white font-medium shadow hover:bg-sky-600 disabled:opacity-80"
            ...
            {/* Error Display */}
            {error && (
              <div className={`rounded-lg p-4 ${isDark ? 'bg-red-900/20 border border-red-700 text-red-300' : 'bg-red-50 border border-red-200 text-red-700'}`}>
