  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignup) {
        ...
        onSuccess?.(userData.user.username || name || nameFromEmail(email), userData.user)
        ...
        // Get user info to pass username
        try {
          const userInfo = await authAPI.getCurrentUser(loginData.access_token)
          onSuccess?.(userInfo.username || nameFromEmail(email), userInfo)
        } catch (userError) {
          // If getting user info fails, still proceed with login
          onSuccess?.(nameFromEmail(email), loginData)
        }
      ...
      if (error.message.includes('Email already registered')) {
        setErrors({ email: 'Email already registered. Please use a different email or log in.' })
      } else if (error.message.includes('Username already taken')) {
        setErrors({ name: 'Username already taken. Please choose a different username.' })
      } else if (error.message.includes('Incorrect email or password')) {
        setErrors({ email: 'Incorrect email or password. Please try again.' })
      } else if (error.message.includes('Inactive user')) {
        setErrors({ email: 'Account is inactive. Please contact support.' })
      } else if (error.message.includes('Failed to fetch')) {
        setErrors({ email: 'Unable to connect to server. Please check your connection.' })
      } else {
